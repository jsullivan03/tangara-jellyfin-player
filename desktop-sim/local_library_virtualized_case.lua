...
local root = "/tmp/tangara-local-library-virtualized"
...
local track_count = positive_integer(
    os.getenv("TANGARA_VIRTUAL_TRACKS") or "118",
    "TANGARA_VIRTUAL_TRACKS"
)
...
local virtual =
    assert(screen.virtual_track_list)

local pool_rows =
    virtual:pool_count()

assert(virtual:logical_count() == track_count)
assert(pool_rows <= 7)
assert(#(screen.media_rows or {}) == pool_rows)
assert(object_delta < 120)
assert(lua_ui_delta > 0)
assert(timer_delta >= 0)

local output_path =
    os.getenv("TANGARA_VIRTUAL_OUTPUT")
...
print(string.format(
    "VIRTUAL tracks=%d create_ms=%.3f pool_rows=%d objects=%d object_delta=%d timers=%d lua_ui_kb=%.1f rss_ui_kb=%d max_rss_kb=%d",
    track_count,
    create_ms,
    pool_rows,
    loaded.active_objects,
